...
function getMean(arr, upper, lower) {
  let total = 0;
  let reject = 0;
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > upper || arr[i] < lower) {
      reject = reject + 1;
      console.log("Rejected: ", arr[i]);
      continue;
    }
    total = total + arr[i];
  }
  return Math.floor(total / (arr.length - reject));
}
...
let algoScores = [90, 90, 90, 90, 90, 90, 90, 90, 90, 0];
...
let heuristicMean = getMean(
  heuristicScores,
  heuristicIQRUpper,
  heuristicIQRLower
);
console.log("IQR Upper is: ", algoIQRUpper);
console.log("IQR Lower is: ", algoIQRLower);
let algoMedian = getMedian(algoScores);
let algoMean = getMean(algoScores, algoIQRUpper, algoIQRLower);
...
describe("Evaluation Contract", async function () {
  ...
  it("should correctly punish evaluators", async () => {
    const selectedEvaluators = await evaluationContract.getEvaluators();
    const evaluatorKey = await evaluatorContract.getEvaluatorKey(
      selectedEvaluators[9]
    );
    const evaluator = await evaluatorContract.getEvaluatorAtIndex(evaluatorKey);
