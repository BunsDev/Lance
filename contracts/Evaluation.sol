...
contract Evaluation {
    ...
    uint256 ownerScore;
    bool ownerSubmitted;
    uint256[] evaluatorAlgorithmicScore = [500, 800, 1500, 2600, 1000, 1800, 300, 1200, 600, 1400, 1100];
    uint256[] evaluatorHeuristicScore = [5, 8, 15, 26, 10, 18, 3, 12, 6, 14, 11];
    uint medianHeuristic;
    uint medianAlgorithmic;
    uint lowerIQRAlgo;
    uint upperIQRAlgo;
    uint lowerIQRHeuristic;
    uint upperIQRHeuristic;
    uint fees = 50;

    IERC20 lanceToken;
    IERC20 bidToken;

    uint256 sharesForMedian = 70;
    uint256 sharesForNonMedian = 30;
    ...
    constructor (address evaluatorContractAddress, IERC20 lanceToken_, IERC20 bidToken_, uint fees_) {
    ...
    function evaluate() public {
        uint averageAlgo = evaluateAlgorithmicTests();
        uint averageHeuristic = evaluateHeuristicTests();
        console.log("Average Algo is: ", averageAlgo);
        console.log("Average Heuristic is: ", averageHeuristic);
    }

    function evaluateAlgorithmicTests() public returns (uint) {
        (lowerIQRAlgo, upperIQRAlgo) = getIQR(evaluatorAlgorithmicScore);
        return calculateAverage(evaluatorAlgorithmicScore);
    }

    function evaluateHeuristicTests() public returns (uint) {
        (lowerIQRHeuristic, upperIQRHeuristic) = getIQR(evaluatorHeuristicScore);
        return calculateAverage(evaluatorHeuristicScore);
    }
    ...
    function getLowerHalf(uint[] memory arr) private pure returns (uint[] memory) {
        ...
        console.log("ArrLength is %s", arrLength);
    ...
    function calculateAverage(uint[] memory arr) private pure returns (uint) {
        uint total = 0;
